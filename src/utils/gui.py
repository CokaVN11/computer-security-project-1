...
class DecryptionStatusFrame(customtkinter.CTkFrame):
    ...
    def create_status_frame(self, tab_name, column=0, row=0):
        ...
        self.sub_frame.grid(row=1, column=0, columnspan=5, padx=(20, 10), pady=10, sticky="nsew")

        self.text_widget = customtkinter.CTkTextbox(self.sub_frame, text_color="#99FF00", font=customtkinter.CTkFont(size=15, family="Courier"), corner_radius=20, fg_color="#000000")
...
class StatusFrame(customtkinter.CTkFrame):
    ...
    def create_status_frame(self, tab_name, column=2):
        ...
        self.status_label.grid(row=0, column=2, columnspan=3, padx=20, pady=10, sticky="ew")

        self.sub_frame = customtkinter.CTkFrame(
            self.tab_view.tab(tab_name),
            # corner_radius=50,
        )
        self.sub_frame.grid(row=1, column=2, rowspan=9, columnspan=3, padx=(20, 10), pady=10, sticky="nsew")
...
class cryptography_app(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.left_sidebar = customtkinter.CTkFrame(self, width=140, corner_radius=0)
        self.left_sidebar.grid(row=0, column=0, rowspan=4, sticky="nsew")
        self.left_sidebar.grid_rowconfigure(4, weight=1)
        self.logo_label = customtkinter.CTkLabel(self.left_sidebar, text="CRYPTOGRAPHY", font=customtkinter.CTkFont(size=25, weight="bold", family="Comic Sans MS"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(40, 10))
        
        # load logo image
        logo_image = customtkinter.CTkImage(light_image = Image.open("src/assets/logo.png"), size=(200, 200))
        my_logo = customtkinter.CTkLabel(self.left_sidebar, image=logo_image, text="")
        ...
        # appearance mode
        self.appearance_mode_label = customtkinter.CTkLabel(self.left_sidebar, text="Appearance Mode:", anchor="w")
        ...
        self.main_frame_1.grid(row=0, column=1, padx=20, pady=(10, 2), sticky="nsew")
        ...
        self.create_folder_selection_widgets("Input File", self.main_frame_1, row=1)
        self.create_folder_selection_widgets("Output Folder", self.main_frame_1, row=2)

        # Create tab view 
        self.tab_view = customtkinter.CTkTabview(self, corner_radius=10)
        self.tab_view.grid(row=1, column=1, padx=(10,10), pady=(5, 10), sticky="nsew")
        ...
        self.genKs = self.create_custom_label("Encryption", "∙ Generate Ks Key and Encryption", 0, 0)
        self.encrypt_button = self.create_custom_button("Encryption", "ENCRYPT with AES", self.encrypt_with_aes, 0, 1)
        self.separator = self.create_separator("Encryption", 1, 0, 2)

        # PART 2 - RSA ENCRYPTION
        self.RSA_alg = self.create_custom_label("Encryption", "∙ RSA Algorithm", 2, 0)
        self.RSA_keypair = self.create_custom_button("Encryption", "RSA KEY PAIR", self.encrypt_with_rsa, 2, 1)
        self.RSA_encrypt = self.create_custom_button("Encryption", "RSA ENCRYPT", self.encrypt_with_rsa, 3, 1)
        self.create_separator("Encryption", 4, 0, 2)

        # PART 3 - SAVE METADATA
        # Create radio buttons for file structure
        self.file_structure = self.create_custom_label("Encryption","∙ File Structure", 5, 0)
        self.file_structure_options = ["XML", "JSON", "Plaintext"]  # Replace with your actual options
        self.file_structure_var = self.create_radio_buttons("Encryption", self.file_structure_options, 5, 1)

        # Save metadata
        self.save_metadata = self.create_custom_label("Encryption", "∙ Save Metadata", 6, 0)
        self.save_button = self.create_custom_button("Encryption", "SAVE METADATA", self.encrypt_with_rsa, 6, 1)
        self.create_separator("Encryption", 7, 0, 2)

        # PART 4 - EXPORT KEY PRIVATE
        self.export_key = self.create_custom_label("Encryption", "∙ Export Key Private", 8, 0)
        self.export_button = self.create_custom_button("Encryption", "EXPORT KPrivate", self.export_Kprivate, 8, 1)

        # PART 5 - STATUS BAR ON THE COLUMN 3
        self.StatusFrame = StatusFrame(self.main_frame_1, self.tab_view, tab_name="Encryption" ,column=0, row=5)
        ...
        self.sub_frame_tab2 = customtkinter.CTkFrame(self.tab_view.tab("Decryption"), corner_radius=10, fg_color="#153448")
        self.sub_frame_tab2.grid(row=1, column=0, columnspan=5, padx=10, pady=10, sticky="nsew")
        ...
        # PART 1 - KPrivate
        self.genKs = self.create_custom_label("Decryption", "∙ KPrivate File", 0, 0)

        # Frame for folder path display
        path_display_frame = customtkinter.CTkFrame(self.tab_view.tab("Decryption"), corner_radius=5, fg_color="#153448")
        path_display_frame.grid(row=0, column=1,columnspan=3, padx=(0, 0), pady=5, sticky="ew")
        # File path label
        file_path_label = customtkinter.CTkLabel(path_display_frame, text="", font=customtkinter.CTkFont(size=13, family="Courier"), text_color="white", anchor="w", justify="left")
        file_path_label.grid(row=0, column=1,columnspan=3, padx=(8,0), pady=4, sticky="w")

        # Folder icon label (acts as a button)
        folder_icon_image = customtkinter.CTkImage(light_image=Image.open("src/assets/folder.png"), size=(30, 30))
        folder_icon_label = customtkinter.CTkLabel(self.tab_view.tab("Decryption"), text="", image=folder_icon_image, bg_color="transparent", anchor="w", cursor="hand2", justify="right")
        ...
        folder_icon_label.bind("<Button-1>", lambda event, fp_label=file_path_label, ft="KPrivate File": self.browse_file(fp_label, ft))
        ...
        label_Kprivate = customtkinter.CTkLabel(
            ...
            text_color="#95D2B3",
            font=customtkinter.CTkFont(size=14, weight="normal", family="Courier"),
            anchor="w",
            justify="center"
        )
        ...
        verify_button = customtkinter.CTkButton(
            ...
            fg_color="#FFB4C2",
            text_color="black",
            font=customtkinter.CTkFont(size=13, weight="bold", family="Trebuchet MS"),
            hover_color="#E0A75E",
            width=15,
        ...
        self.DecryptionStatusFrame = DecryptionStatusFrame(self.tab_view.tab("Decryption"), self.tab_view, tab_name="Decryption", column=0, row=3)
        self.DecryptionStatusFrame.sub_frame.grid(row=3, column=0, columnspan=5, padx=10, pady=10, sticky="nsew")
    ...
    def create_radio_buttons(self, tab_name, options, row, column):
        ...
        for i, option in enumerate(options):
            ...
            radio.grid(row=i, column=0, sticky="w", pady=2)  # Adjust pady for vertical spacing
    ...
    def display_student_name(self, student_name: str, student_id: str, row_indx):
        student_frame = customtkinter.CTkFrame(self.left_sidebar, corner_radius=10)
        student_frame.grid(row=row_indx, column=0, padx=15, pady=(8, 5), sticky="ew")
        student_frame.grid_columnconfigure(0, weight=1)
        student_name_label = customtkinter.CTkLabel(student_frame, text=f"{student_id} - {student_name}", font=customtkinter.CTkFont(size=16, weight="bold", family="Candara"), anchor="w")
        student_name_label.grid(row=row_indx, column=0, padx=10, pady=5, sticky="w")

    def create_folder_selection_widgets(self, label_text, frame, row):
        if label_text == "String Input":
            # Label for folder selection
            label = customtkinter.CTkLabel(frame, text=label_text, font=customtkinter.CTkFont(size=14, weight="bold", family="Comic Sans MS"))
            label.grid(row=row, column=0, sticky="w", padx=(25, 0), pady=(10, 5))
            # string input entry
            entry = customtkinter.CTkEntry(frame, font=customtkinter.CTkFont(size=12))
            entry.grid(row=row, column=1, padx=(0, 0), pady=(10, 5), sticky="ew")

            # upload button
            upload_button = customtkinter.CTkButton(frame, text="Upload", corner_radius=5, command=self.upload_content)
            upload_button.grid(row=row, column=2, padx=(10, 0), pady=(10, 5))
            return
        
        # Label for folder selection
        label = customtkinter.CTkLabel(frame, text=label_text, font=customtkinter.CTkFont(size=14, weight="bold", family="Courier New"))
        label.grid(row=row, column=0, sticky="w", padx=(25, 0))

        # Frame for folder path display
        path_display_frame = customtkinter.CTkFrame(frame, corner_radius=5)
        path_display_frame.grid(row=row, column=1, padx=(0, 0), pady=5, sticky="ew")
        path_display_frame.grid_columnconfigure(0, weight=1)

        # File path label
        file_path_label = customtkinter.CTkLabel(path_display_frame, text="", font=customtkinter.CTkFont(size=12), anchor="w", justify="left")
        file_path_label.grid(row=0, column=1, padx=10, pady=4, sticky="w")

        # Folder icon label (acts as a button)
        folder_icon_image = customtkinter.CTkImage(light_image=Image.open("src/assets/folder.png"), size=(35, 35))
        folder_icon_label = customtkinter.CTkLabel(frame, text="", image=folder_icon_image, anchor="w", cursor="hand2")
        folder_icon_label.grid(row=row, column=2, padx=(0, 10), pady=5)

        # Bind the folder icon label to the browse_file method with the corresponding arguments
        folder_icon_label.bind("<Button-1>", lambda event, fp_label=file_path_label, ft=label_text: self.browse_file(fp_label, ft))
    ...
    def create_custom_label(self, tab_name, label_text, row_indx, column_indx):
        ...
        label.grid(row=row_indx, column=column_indx, padx=10, pady=10, sticky="ew")
        return label
    ...
    def create_custom_button(self, tab_name, button_text, button_command, row_indx, column_indx):
        ...
        button.grid(row=row_indx, column=column_indx, padx=10, pady=10, sticky="ew")
        return button

    def create_separator(self, tab_name, row, column, columnspan):
        separator = customtkinter.CTkFrame(self.tab_view.tab(tab_name), height=2, fg_color="gray")
        separator.grid(row=row, column=column, columnspan=columnspan, sticky="ew", padx=10, pady=10)
        return separator
